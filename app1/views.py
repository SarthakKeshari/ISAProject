...
def modify_file(filename, clientid):
    ...
    img = cv2.imread('./media/documents/R_2.jpg')
	# print(img)
    ...
    watermark = PdfFileReader(open("./media/documents/watermark.pdf", "r+b"))
    ...
    outputStream = open("./media/documents/document-output.pdf", "w+b")
...
def checkdocument(request):
    if(k):
        messages.error(request, "Culprit: "+ k.pop()+ " is culprit")
    try:
        username = request.session['username']
        designation = request.session['access']
    except:
